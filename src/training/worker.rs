use super::{TrainingCommand, TrainingEvent, Trainer};
...
impl TrainingWorker {
    pub fn spawn(mut trainer: Trainer, control_file: Option<PathBuf>) -> Self {
        ...
        Self { commands: command_tx, events: event_rx, join: Some(join) }
    }

    pub fn send(&self, command: TrainingCommand) -> Result<(), String> {
        self.commands.send(command).map_err(|e| format!("training command send: {e}"))
    }
