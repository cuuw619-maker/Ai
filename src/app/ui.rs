...
impl AiApplication {
    ...
    fn home(&mut self, ui: &mut Ui) {
        ...
        ui.horizontal_wrapped(|ui| {
            self.metric_card(
                ...
                "MODEL",
                self.model_status(),
                model_detail(self.core.model.as_ref()),
            );
            self.metric_card(
                ui,
                "TRAINING",
                ...
                format!(
                    "step {} • epoch {}",
                    self.core.training.step, self.core.training.epoch
                ),
            );
            self.metric_card(
                ui,
            ...
            self.metric_card(
                ...
                dataset_detail(self.core.dataset.as_ref()),
            );
        });
        ui.add_space(10.0);
        ui.horizontal(|ui| {
            if ui
                .add_sized([130.0, 36.0], egui::Button::new("CHAT"))
                .clicked()
            {
                self.core.command_page(AppPage::Chat);
            }
            if ui
                .add_sized([130.0, 36.0], egui::Button::new("TRAIN"))
                .clicked()
            {
            ...
            let can_pause = matches!(
                ...
                crate::training::TrainingStatus::Running
            );
            if ui
            ...
            let can_resume = matches!(
                self.core.training.state,
                crate::training::TrainingStatus::Paused | crate::training::TrainingStatus::Stopped
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                if self.core.is_trainer_running() {
                    ...
                    self.core.training.state,
                    crate::training::TrainingStatus::Paused
                        | crate::training::TrainingStatus::Stopped
                ) && self.core.training.checkpoint.is_some()
                {
        ...
        ui.columns(2, |columns| {
            ...
            card(&mut columns[1], |ui| {
                ...
                row_value(
                    ui,
                    "Memory budget",
                    &format!("{} MB", self.core.config.training.memory_budget_mb),
                );
                row_value(
                    ui,
                    "Checkpoint",
                    &format!(
                        "every {} steps",
                        self.core.config.training.checkpoint_interval_steps
                    ),
                );
            });
        });
    ...
    fn model(&mut self, ui: &mut Ui) {
        ...
        let memory = if params > 0 {
            params.saturating_mul(4)
        } else {
            0
        };
        ui.label(format!("Estimated parameters: {}", params));
        ui.label(format!(
            "Estimated weights memory: {}",
    ...
    fn dataset(&mut self, ui: &mut Ui) {
        ...
        card(ui, |ui| {
            ...
            let vocab = self
                ...
                .tokenizer_path
                .as_ref()
                .and_then(|p| crate::tokenizer::Tokenizer::load(p).ok())
                .map(|t| t.vocab_size());
            row_value(
                ui,
                "Vocabulary",
                &vocab.map(|v| v.to_string()).unwrap_or_else(|| "—".into()),
            );
            row_value(
                ui,
                "Special tokens",
                if vocab.is_some() { "7" } else { "—" },
            );
        });
    }
    ...
    fn evaluation(&mut self, ui: &mut Ui) {
        ...
        card(ui, |ui| {
            ...
            row_value(
                ui,
                "Weights mutated",
                "No — evaluation uses forward-only sequence_loss",
            );
    ...
    fn logs(&mut self, ui: &mut Ui) {
        ...
        ui.horizontal(|ui| {
            for name in ["APP LOG", "TRAINING LOG", "INFERENCE LOG", "CRASH LOG"] {
                if ui
                    .selectable_label(self.core.log_channel == name, name)
                    .clicked()
                {
    ...
    fn system(&mut self, ui: &mut Ui) {
        ...
        subsystem(
            ui,
            "Training",
            self.core.worker.is_some()
                || !matches!(
                    self.core.training.state,
                    crate::training::TrainingStatus::Idle
                ),
            true_status(&self.core.training),
        );
            ...
            ui.horizontal(|ui| {
                let response = ui.add_sized(
                    ...
                    egui::TextEdit::multiline(&mut self.core.chat_input)
                        .hint_text("Ask the local AiNet model..."),
                );
        ...
        if self.core.previous_crash {
            egui::Window::new("Previous crash detected")
                ...
                .show(ctx, |ui| {
                    ...
                    ui.label(
                        "Safe Mode does not auto-load the last model and does not start training.",
                    );
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.core.tokenizer_job_rx.is_some() {
            egui::Area::new("tokenizer-job".into())
                ...
                .show(ctx, |ui| {
                    card(ui, |ui| {
                        ui.label("Tokenizer job is running in background...")
                    });
                });
        }
...
fn status_pill(ui: &mut Ui, value: &str) {
    let response = ui.add(
        egui::Label::new(RichText::new(value).monospace().strong()).sense(egui::Sense::hover()),
    );
    if response.hovered() {
...
fn true_status(training: &super::core::TrainingSnapshot) -> &str {
    if training.state == crate::training::TrainingStatus::Failed {
        "Training failed"
    } else {
        training.label()
    }
...
fn estimate_parameters(vocab: usize, embedding: usize, hidden: usize, layers: usize) -> usize {
    ...
    let cell = layers.saturating_mul(
        hidden
...
fn draw_loss_graph(ui: &mut Ui, points: &std::collections::VecDeque<(u64, f32)>) {
    ...
    for (step, loss) in points {
        let x = inner.left()
            + ((*step - min_step) as f32 / (max_step - min_step) as f32) * inner.width();
